#!/usr/bin/env python3
"""
SHARP with RadDINO encoder support - Workshop resubmission robustness check

Adds --encoder_type argument to swap ImageNet ViT with domain-specific RadDINO.
All other functionality identical to train_sharp_large_batch.py.

Usage:
    # ImageNet ViT baseline (Exp #1)
    python train_sharp_raddino_v2.py --encoder_type vit --output_dir ./exp_raddino_baseline

    # RadDINO + hard negatives (Exp #3) - MOST IMPORTANT
    python train_sharp_raddino_v2.py --encoder_type raddino --hard_neg_max_frac 0.6 --output_dir ./exp_raddino_hardneg
"""
...
class ImageEncoderViT(nn.Module):
    def __init__(self, embedding_dim=256, encoder_type="vit"):
        super().__init__()
        self.encoder_type = encoder_type

        if encoder_type == "raddino":
            print("Loading RadDINO encoder (microsoft/rad-dino)...")
            ...
            try:
                ...
                print("   [OK] Loaded RadDINO (domain-specific chest X-ray encoder)")
            except Exception as e:
                raise RuntimeError(
                    f"Could not load RadDINO: {e}\n"
                    "Ensure it's downloaded or run: huggingface-cli download microsoft/rad-dino"
                )
            ...
            raise ValueError(f"Unknown encoder_type: {encoder_type} (choices: 'vit', 'raddino')")
...
class GraphTextCLIP(nn.Module):
    def __init__(self, embed_dim=256, temperature=0.07, encoder_type="vit"):
        super().__init__()
        self.image_encoder = ImageEncoderViT(embed_dim, encoder_type=encoder_type)
...
def main(args):
    ...
    # -- 7. Model - start from ImageNet weights or RadDINO
    print(f"\nCreating model with {args.encoder_type} encoder...")
    model = GraphTextCLIP(embed_dim=256, temperature=0.07, encoder_type=args.encoder_type).to(device)
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(description="SHARP with RadDINO support")

    # NEW: Encoder selection
    p.add_argument("--encoder_type", choices=["vit", "raddino"], default="vit",
                   help="Image encoder: 'vit' (ImageNet) or 'raddino' (domain-specific)")
