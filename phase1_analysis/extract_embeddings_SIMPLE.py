"""
Phase 1: Extract embeddings - STANDALONE VERSION

This script has ZERO external dependencies (except standard libraries).
It will work because:
1. All model code is included directly (no imports from training script)
2. Uses correct parameter names (embedding_dim=256, not d_model=128)
3. Only requires: torch, transformers, PIL, numpy
"""
...
# ============================================================================
# Model Definitions (copied directly, no imports needed)
# ============================================================================
...
def get_study_id_from_path(scene_path):
    ...
    # Path format: .../pXX/pXXXXXXX/sXXXXXXXX.scene_graph.json
    parts = Path(scene_path).stem.split('.')
    return parts[0]  # sXXXXXXXX
...
def load_image(dicom_id, image_dir):
    ...
    # Find image file (format: pXX/pXXXXXXX/sXXXXXXXX/XXXXXXXX.jpg)
    p_prefix = f"p{dicom_id[:2]}"
    subject_id = dicom_id.split('_')[0]  # pXXXXXXX
    study_id = dicom_id.split('_')[1]    # sXXXXXXXX
    img_file = dicom_id.split('_')[2]    # XXXXXXXX
