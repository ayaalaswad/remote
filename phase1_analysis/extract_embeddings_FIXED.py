"""
Phase 1: Extract embeddings - FIXED VERSION

Fixed issues:
1. Study ID extraction from scene paths
2. Debug output to show what's happening
...
def load_split_csv(csv_path):
    ...
    for _, row in df.iterrows():
        # Study ID in CSV might be int or have 's' prefix
...
def get_study_id_from_path(scene_path):
    ...
    Path formats:
    - .../pXX/pXXXXXXX/sXXXXXXXX.scene_graph.json
    - .../sXXXXXXXX.scene_graph.json
    """
    stem = Path(scene_path).stem  # Remove .json
    # Remove .scene_graph suffix
    if '.scene_graph' in stem:
        stem = stem.split('.scene_graph')[0]

    # Should now have sXXXXXXXX
    if stem.startswith('s'):
        return stem

    # Try extracting from path components
    parts = Path(scene_path).parts
    for part in reversed(parts):
        if part.startswith('s') and len(part) >= 8:
            return part.split('.')[0]
...
def partition_files(scene_files, study_to_split, study_to_subject):
    ...
    print(f"\nPartitioning {len(scene_files)} scene files...")
    print(f"CSV has {len(study_to_split)} study IDs")

    # Debug: show first few study IDs from each source
    print(f"\nFirst 5 study IDs from CSV: {list(study_to_split.keys())[:5]}")
    if scene_files:
        print(f"First 5 scene files: {scene_files[:5]}")
        print(f"Extracted study IDs: {[get_study_id_from_path(f) for f in scene_files[:5]]}")
    ...
    print(f"  Subject conflicts: {subject_conflicts}")
...
def load_image(dicom_id, image_dir):
    """Load and preprocess image"""
    from torchvision import transforms

    # Parse dicom_id: format could be pXXXXXXX_sXXXXXXXX_XXXXXXXX
    ...
    p_prefix = f"p{subject_id[1:3]}"  # p10 from p10000032
    img_path = Path(image_dir) / p_prefix / subject_id / study_id / f"{img_file}.jpg"
    ...
    with torch.no_grad():
        for scene_file in tqdm(val_files[:max_samples]):
            try:
                ...
                # Load image
                img = load_image(dicom_id, image_dir)
                if img is None:
                    continue
...
def main():
    ...
    if not val_files:
        ...
        print("This usually means study ID extraction is broken.")
        print("Please check scene file naming convention.")
        return
