...
projection head.
Although Stage~1 trains on bounding-box crops, the ViT-B/16 positional
embeddings are interpolated to full-image resolution at the start of
Stage~2 fine-tuning, following standard ViT transfer practice.
The 10-epoch fine-tuning stage allows the encoder to adapt its spatial
representations to the full-image regime before evaluation.
The full encoder--decoder is fine-tuned end-to-end on MIMIC-CXR
($\eta{=}5{\times}10^{-5}$, 10 epochs, AdamW).
...
further.
Reviewer note: Symmetric InfoNCE (A) constitutes a stronger and fairer
baseline than ImageNet-FT for assessing SHARP's specific contributions,
since both (A) and SHARP train on identical data.
The $+2.0\,\%$ gain of SHARP over (A) isolates the effect of multi-positive
grouping and curriculum pacing specifically.
...
Conditions with smaller absolute gains --- Fracture ($+0.012$), Lung Lesion ($+0.011$), and Consolidation --- reflect findings where visual presentation is highly variable and polarity distinctions are less discrete, limiting the benefit of negation-hard negatives specifically.
